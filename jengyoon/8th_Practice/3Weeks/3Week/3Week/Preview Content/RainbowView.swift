...
struct RainbowView: View {
            ...
            ForEach(RainbowModel.allCases, id: \.self, content: { rainbow in
                makeColorCard(rainbow)
                    .onTapGesture {
                        ...
                        self.navigationTrue.toggle() // 클릭시 네비게이션 활성화 되도록 toggle()로 넘기기
                    }
            })
        })
    }
    ...
    private var bottomSelectedColorGroup: some View {
        VStack(spacing: 70, content: {
            Image(.appleLogo)
                .renderingMode(.template) // 지정해야 색 바뀜
                .fixedSize()
                .foregroundStyle(viewModel.selectedRainbowModel?.returnColor() ?? Color.black)
