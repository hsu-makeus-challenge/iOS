...
class MenuDetailViewModel: ObservableObject {
    ...
    @Published var selectedTemperatureIndex: Int
    ...
    init(
        ...
        // 초기 온도 타입에 맞는 인덱스를 찾아서 설정
        if let index = menu.menuTemperatureItem.firstIndex(where: {
            $0.temperatureType == selectedTemperatureType
        }) {
            self.selectedTemperatureIndex = index
        } else {
            self.selectedTemperatureIndex = 0 // fallback
        }
    }
